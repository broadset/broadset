...
- **External PDF/A validator (e.g. veraPDF) integration.** The bundled `validatePdfA2b` is a homegrown structural check enforcing: presence of `/OutputIntents` + `/GTS_PDFA1`, trailer `/ID` array, `pdfaid:` XMP packet, no `/Encrypt`, no `/JavaScript`, no `/LZWDecode`, embedded-font `/ToUnicode` CMaps (skipping CIDFont descendants and Standard-14 unembedded fonts), and `/Group /S /Transparency` declaration on pages that use transparency operators. The check does NOT exercise every ISO 19005-2 clause veraPDF audits (e.g. blending colour spaces, full font-program validation, transparency-group nested isolation). PDF/A conformance for arbitrary documents is therefore self-graded against the structural floor; veraPDF integration is a deliberate follow-up that requires bundling the veraPDF JAR or a WASM port.
- **Adobe / Microsoft / Apple binary fixtures.** Importer coverage against actual producer binaries (Illustrator `.ai` PDFs, Word `.pdf` exports, macOS Preview saves, Acrobat-edited files) requires licensing diligence + storage budget for committed binaries. As a local stand-in the importer is exercised against THREE real PDF generators piped through their own pipelines:
  ...
  - [`cross-producer-features.test.ts`](../../../packages/formats/src/pdf/cross-producer-features.test.ts) — `jsPDF` (MIT) generated fixtures: distinct PDF byte format from pdfkit (no FlateDecode by default, unusual transformation matrices, decimal coordinates) plus pdfkit feature coverage (date metadata, Subject/Keywords, uncompressed streams, banner-aspect pages). Plus chain round-trip across producers.
  - The cross-producer suite caught a real importer bug — `/OpenAction` as a destination array (jsPDF default) crashed `hasEmbeddedJavaScript` because pdf-lib's `lookupMaybe(..., PDFDict)` throws on type mismatch. Fixed in this same pass.

  Committing additional binary fixtures from Adobe / Microsoft / Apple producers when redistribution is permitted remains a follow-up.
