...
- **Real-world third-party fixture corpus.** The importer is exercised against the [`producer-quirks.test.ts`](../../../packages/formats/src/pdf/producer-quirks.test.ts) suite which programmatically synthesises the structural quirks Acrobat / Word / pdflatex / Illustrator / Figma / macOS Preview emit (multi-stream `/Contents` arrays, missing trailer `/ID`, non-Latin `/Producer` info, no `broadset:` XMP, FlateDecode-compressed streams, `useObjectStreams: true` packing). Real producer binaries are NOT committed because of licensing diligence + storage budget; the synthetic suite is the local stand-in. Real-fixture coverage remains a follow-up.
