...
- **Importer regex parsing (acceptance: "The importer does NOT use regex to extract XML attributes or element content").** The `pptx/import/*` and `pptx/semantic/*` modules use regex extensively for attribute and element-block extraction. fast-xml-parser is used for XXE / billion-laughs hardening (every XML part is sanity-parsed before the regex hot path runs) but the operator-level shape walker, transform extractor, run/paragraph parser, and ledger / element-ext readers all use regex. The block extractor is depth-aware (`findMatchingCloseIdx` in `pptx/import/shapes.ts`), so nested same-named tags resolve correctly; entity-encoded attribute values, entity-encoded text, whitespace variations, self-close trailing whitespace, and XML comments all parse correctly today (verified by `pptx/import-edge-cases.test.ts`). **Known regex limitations** (5 entries) are pinned as `it.skip` tests in `pptx/import-edge-cases.test.ts` and gated by an `EXPECTED_SKIPPED_PROBES` constant so silent growth fails CI: non-default namespace prefixes (`<dml:sp>` instead of `<p:sp>`), CDATA-wrapped text, attribute-order swap on `<a:off>`, single-quoted attribute values, and decimal `<a:pt>` coordinates inside `<a:custGeom>`. Tracked as a Phase 8.x follow-up that will rebuild the import pipeline on a fast-xml-parser AST walker; rebuild gate is "every skipped test in `import-edge-cases.test.ts` passes (and the skip-count guard reads zero)".
- **CMYK / display-p3 colour preservation on import.** Today's importer resolves CMYK colours to sRGB at the model boundary (Broadset doesn't currently store CMYK directly); display-p3 inputs are similarly normalized. `<a:scrgbClr>` (linear-light), `<a:hslClr>` (HSL components), and `<a:prstClr>` (preset names per ECMA-376 §20.1.10.46) parse with the source form preserved on `BroadsetColor.originalColor`, and `emitColorFill` re-emits the same primitive on export so the round-trip is lossless (verified by `pptx/import.test.ts` — "round-trips scrgbClr / hslClr / prstClr colour primitives"). The export side also gates `<a:prstClr val="…"/>` against the canonical ECMA-376 enum via `pptx/ooxml/preset-colors.ts` so a stale or crafted `originalColor` cannot produce a `.pptx` PowerPoint refuses with a repair dialog.
- **`<a:blipFill>` `srcRect` pixel-level crop baking.** OOXML `<a:srcRect>` crops are preserved on `extensions.pptx.srcRect` and re-emitted on export (so a round-tripped `.pptx` keeps PowerPoint's view of the image). Pixel-level baking — actually cropping the source image bytes so the rendered Broadset element shows only the cropped region — is deferred until the renderer adopts a proper crop primitive on image elements; for now Broadset shows the full image while PowerPoint shows the cropped view. Tracked as a renderer-side follow-up.
- **Gradient slide backgrounds.** Solid `<p:bg>` fills round-trip losslessly. Gradient `<p:bg>` fills downgrade to the first stop colour and surface an `unsupported-content` warning, because the canvas model carries a single solid colour today. Native gradient canvas backgrounds require a model-level addition tracked as a follow-up unit.
- **Multi-page ledger identity disambiguation.** When a user duplicates a slide in PowerPoint, the duplicated shapes share the source BSET id; the current ledger merge keys on `el.id` only, so the two instances collapse to one. Pinned as a skipped test in `pptx/import.test.ts` ("preserves duplicated slides through the merge ledger"). The fix requires keying on `(slideId, elementId)` and is tracked as a Phase 8.x follow-up.
