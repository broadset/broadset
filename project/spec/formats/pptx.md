...
- **`<a:innerShdw>` (inset shadows).** Only `<a:outerShdw>` round-trips through `style.boxShadow`. CSS `inset` shadows are silently skipped on export because OOXML's outer-shadow primitive cannot represent them; layered shadow stacks fall back to the first non-inset entry. Surfacing a structured warning on the importer / exporter remains future work — today the drop is silent.
- **Export warnings sink.** The exporter has no `warnings` channel. Drops surfaced during export (animation presets beyond fade-entry per IO-D-16, multi-shadow truncation, inset-shadow skips, unsupported gradient stops, etc.) are taken silently. Future work: add a structured warnings array to the exporter result so callers can show a fidelity-loss toast like the importer does.
