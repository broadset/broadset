...
- **Font embedding under `ppt/fonts/`.** Subsetted embedded fonts use the `_shared/fonts/` subsetter landed in Phase 4. Full font-embedding acceptance tests (byte-level OS/2 / name-table assertions) land with the external-tool fixture work in Phase 5 when a license-clear font fixture is available.
...
- **PowerPoint "Check for Issues" CI gate.** The canonical-fixture "zero warnings" assertion uses LibreOffice headless as a stand-in in CI (PowerPoint not available on Linux runners). A manual-gated PowerPoint-on-Windows test is documented as a nightly job in the plan.
