...
- **`<a:innerShdw>` (inset shadows).** Only `<a:outerShdw>` round-trips through `style.boxShadow`. CSS `inset` shadows are skipped on export because OOXML's outer-shadow primitive cannot represent them; layered shadow stacks fall back to the first non-inset entry. The exporter now surfaces this through the `shadow-inset-skipped` and `shadow-truncated` warnings via {@link exportPptxWithReport}.
