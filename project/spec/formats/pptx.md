...
- **Font embedding under `ppt/fonts/`.** The `_shared/fonts/` subsetter (Phase 4) emits byte-minimal subsets that preserve the OS/2, name, and post tables required by PPTX `<p:embeddedFontLst>` and PDF `/FontDescriptor` (fontkit's CFF / TTF subsetter strips them; we copy them through a verbatim post-process). Byte-level acceptance tests run against the committed Codicon (MIT) fixture in `_shared/fonts/__fixtures__/`; see `_shared/fonts/subset.test.ts` for OS/2 fsType + weight + width and name family / subfamily / postscript assertions on the subset output. Wiring the subsetter through the PPTX exporter (`<p:embeddedFontLst>` part emission) remains a follow-up unit.
