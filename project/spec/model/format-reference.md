...
| `loadProjectV1Json`                        | Bounded JSON-source loading with typed quarantine and source-byte preservation    |
...
- [ ] Given a structurally or semantically invalid JSON source, loading returns typed diagnostics and the exact original bytes without inserting defaults
...
All selected ordinary page roots in one document MUST be partitioned from one memoized hierarchy
traversal; adding independent roots MUST NOT trigger a full document-element scan per root.
...
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `element`       | `/geometry/bounds/width\|height` → `length`; `/geometry/origin` → `point3d`; affine `/geometry/transform/matrix/0..3` → `number`, `/4..5` → `length`; matrix3d `/0..11\|15` → `number`, `/12..14` → `length`; `/appearance/opacity` → `number`; `/accessibility/label\|description` → `string`; image/video/audio asset ID → `asset`; video autoplay/loop/muted/controls and audio autoplay/loop → `boolean`; audio volume → `number`; clock format/timeZone/locale → `string`; ticker direction → `string`, speed/gap → `number`, repeat → `boolean`; QR value/errorCorrection → `string`, quietZone → `length`; image focalPoint → `point2d`; text layout columns → `integer`, columnGap and padding entries → `length`, verticalAlignment/overflow/autoSize → `string`, textPath startOffset → `length`; group clipChildren → `boolean`; foreign previewAssetId → `asset` |
...
The model-owned `resolveCanonicalSceneV1` boundary MUST validate the whole project before returning an
owned, recursively frozen canonical scene. It resolves definition, component-property, page-root, and
page-override layers, composite parent addresses, canonical paint order, selected variable/sample maps,
resource context, and exact affine/matrix3d world geometry. Binding, state, sequence, lifecycle, and
exact-tick orchestration remain playback-owned and complete `ResolvedSceneSnapshotV1` without making
consumers merge persisted layers independently.
