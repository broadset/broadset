...
- [ ] Given a timeline is open, the toolbar visibly names the timeline context and play state
- [ ] Given a timeline is open, Play/Pause and Reset accessible labels name that timeline
- [ ] Given a timeline is opened from the toolbar, document-level playback is stopped first
...
The 16px side inset is the minimum inset. The demo shell MUST increase the left inset to clear the persistent element toolbar, and MUST increase the right inset to clear an open sidebar drawer. Timeline panel controls, rows, and the close button MUST NOT render underneath side rails or the sidebar.
...
| `position`      | `fixed`                                                                                         |
| `bottom`        | `0`                                                                                             |
| `left`          | At least `16px` (`TIMELINE_BOTTOM_PANEL_SIDE_INSET_PX`), increased to clear left chrome         |
| `right`         | At least `16px` (`TIMELINE_BOTTOM_PANEL_SIDE_INSET_PX`), increased to clear open sidebar chrome |
...
- GIVEN a timeline is opened for editing
- WHEN the panel renders
- THEN it is fixed at the bottom with at least 16px side insets and does not overlap persistent side chrome

#### Acceptance Criteria

- [ ] Given a timeline being edited, the TimelineBottomPanel is visible at the bottom
- [ ] Given the timeline panel is open, it is fixed-positioned at `bottom: 0` with side insets of at least 16px
- [ ] Given the element toolbar or sidebar is visible, the TimelineBottomPanel insets keep its content clear of that chrome
