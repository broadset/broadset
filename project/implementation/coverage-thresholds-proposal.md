...
## Recommendation: defer gating until RQ-3 + RQ-5..RQ-8 land

Adding coverage thresholds to `gate:full` today would freeze regressions that *raise* the baseline (good) but also lock in distortions caused by:
...
Wait for the cross-region CT audit (RQ-3) and producer-compatibility runs (RQ-5..RQ-8) to fill those rows, then re-evaluate.
...
1. The PPTX producer-compatibility stubs (RQ-5) will populate `chart.ts` / `table.ts` and add fixture rows for `import/group.ts` etc. — those will move the formats numbers up; locking the floor at today's baseline would gate every PR that bumps coverage temporarily as new stubs land.
2. Cross-region CT (RQ-3) will likely move some demo files from "0 unit coverage, full CT coverage" to "still 0 unit coverage, still full CT coverage" — the unit-coverage signal stays misleading until the CT audit closes.
...
**Re-evaluation trigger:** revisit this proposal once RQ-3, RQ-5, RQ-6, RQ-7, and RQ-8 are all checked in `release-quality-closure-plan.md`. At that point:
...
- Re-evaluation trigger named: RQ-3 + RQ-5..RQ-8 closure.
