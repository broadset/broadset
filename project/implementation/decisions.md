...
### Unit 5.6 — ag-psd for PSD export/import with duplicated blend constants

**Decision:** Used `ag-psd` library for PSD read/write. Duplicated the CSS↔PSD blend-mode mapping table (~20 lines) in both `psd.ts` (export) and `psd-import.ts` (import) to avoid circular dependency after file split.

**Alternatives considered:** (a) Extract shared constants to `psd-shared.ts` — adds a fourth file for ~20 lines of data, over-engineered. (b) Keep everything in one file — violates 500-line soft limit. (c) Have `psd-import.ts` import from `psd.ts` — creates a circular dependency since `psd.ts` re-exports from `psd-import.ts`.

**Rationale:** The duplication is minimal (a static lookup table) and the two copies are generated from the same algorithm (invert the CSS→PSD map). If the mapping ever needs updating, both files must change, but this is a rare event and the data is self-documenting.
