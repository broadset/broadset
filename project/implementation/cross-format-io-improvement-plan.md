...
**Architecture:** Three execution waves. **Wave 1 (Phases 1–3)** — **done** on `dev2-phase-11`: wired UI for format I/O, closed SVG security findings, consolidated shared utilities. **Wave 2 (Phase 4)** — per-format feature parity (PSD CMYK, PSD effects, PDF ICC CMYK, …); sub-plans remain for open items — **CFIO 4.2 (PDF shading + per-element OCG) is done**. **Wave 3 (Phase 5)** — operational hardening (telemetry done; visual CI, licensed fixtures, a11y audits open).
...
| [project/spec/formats/pdf.md](../spec/formats/pdf.md) §Spec Gaps | Real ICC CMYK/Lab/spot emission (Phase 4.1). **Closed:** shading patterns, per-element OCG, font subsetting (CFIO 4.2, 5.8) |
...
## 4.2: PDF P6.3 — real type-2 / type-3 shading patterns + per-element OCG wrappers — DONE

**Closed on `dev2-phase-11`.** Shading patterns emit via [`packages/formats/src/pdf/export/shading.ts`](../../packages/formats/src/pdf/export/shading.ts); per-element `/OC … BDC … EMC` wrappers bind through [`export/ocg.ts`](../../packages/formats/src/pdf/export/ocg.ts) `bindingByElementId` and [`core.ts`](../../packages/formats/src/pdf/core.ts). Verified by [`shading-and-ocg.test.ts`](../../packages/formats/src/pdf/shading-and-ocg.test.ts), [`fidelity-upgrades.test.ts`](../../packages/formats/src/pdf/fidelity-upgrades.test.ts), and [`export-beyond-prior-art.test.ts`](../../packages/formats/src/pdf/export-beyond-prior-art.test.ts). Conic gradients retain the raster fallback path by design.
...
- [x] Phase 1 — UI wiring (export options, reconciliation generalization, preflight modal)
- [x] Phase 2 — SVG security closures (H2, M2)
- [x] Phase 3 — Shared utility extraction (text-layout, effects, geometry)
- [ ] Phase 4 — Per-format feature parity (sub-plans) — 4.2 done; 4.1, 4.3, 4.6 open
- [ ] Phase 5 — Operational hardening (sub-plans) — 5.4, 5.6 done; 5.1, 5.3, 5.5, 5.7 open
- [x] Phase 6 — Gap-file rolling updates (ongoing; see plan-progress.md DOC.1)
```
