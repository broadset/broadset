...
# Properties Panel Parity + UX Upgrade Plan (Ralph Loop, Hard-Guarded)

**Packages:** `packages/ui`, `packages/editor`, `packages/demo`, `packages/model` (if schema/types required)  
**Reference baseline:** `../dom-compositor/packages/heroui/src`  
**Primary spec anchors:** `project/spec/ui/panels.md`, `project/spec/ui/inputs.md`, `project/spec/editor/spec.md`  
**Related phase plan:** `project/implementation/plan-phase-5.md`  
**Execution model:** Ralph loop per unit (`tests: red` -> `impl: green` -> `refactor: done`)
...
This section exists to prevent implementation drift and low-quality shortcuts.

### 0.1 Hard rules

1. Never ship a panel behavior that is not covered by tests.
2. Never weaken lint/type/test gates to force green.
3. Never replace HeroUI components with raw HTML controls where HeroUI equivalents exist.
4. Never change model semantics silently to fit UI convenience.
5. Never mark a unit complete unless all required commands pass.
6. Never modify unrelated files in the same commit.

### 0.2 Forbidden shortcuts (explicit anti-stupidity list)

- No `@ts-ignore`, no `@ts-expect-error`, no eslint-disable comments.
- No "TODO implement later" in production code for required behavior.
- No fake behavior via toasts/placeholders when spec expects real state mutation.
- No asserting only one region when scenario is cross-region (panel -> canvas/timeline/layers).
- No brittle selector strategy in tests (use role/name/testid patterns already used in repo).
- No undocumented new props or side effects in exported APIs.

### 0.3 Definition of Done (global)

A unit is done only when all are true:

- `tests: red` committed/validated first
- `impl: green` complete and minimal
- `refactor: done` with no behavior regressions
- Required package quality gates pass
- Required CT gates pass at milestone boundaries
- This file progress table updated
- Relevant spec docs updated additively when behavior clarification is introduced
...
Close all meaningful feature and UX gaps between current properties panel behavior and prior `dom-compositor` behavior, while retaining Broadset model architecture and HeroUI standards.

### 1.2 Required outcomes

- Missing feature parity restored where still valid for Broadset
- UX improvements implemented without regressions
- Capability-driven panel visibility remains correct
- Screen vs print constraints remain correct
- Animation-mode property editing parity restored
- Demo clearly demonstrates all upgraded workflows

### 1.3 Out of scope (for this plan)

- New element types not already in model capability matrix
- Full redesign of sidebar IA unrelated to parity/UX goals
- Broadset-owned backward-compatibility shims unless explicitly required by spec
...
1. `project/spec/*` files
2. `project/implementation/architecture.md` package boundaries
3. HeroUI rules in `.github/instructions/heroui.instructions.md`
4. Previous app (`../dom-compositor`) as behavior reference

The previous app is reference behavior, not automatic authority. Keep only behaviors aligned with current specs and model constraints.

---

## 3) Gap Inventory (Confirmed Targets)

### A. Sidebar context and edit safety

- Selected element context header (name + type chip)
- Lock/unlock control in header
- Locked-state helper + full control disable behavior

### B. Geometry workflow

- Element name editing in geometry context
- Horizontal/vertical anchor controls
- Unit-aware geometry labels and conversions (px/mm/in)

### C. Appearance workflow

- Rich gradient editing (mode + stops + angle)
- Full border radius corner editing (linked/unlinked)
- Better border/fill grouping

### D. Typography and text effects

- Fast toggles for common formatting actions
- Better advanced grouping/discoverability
- Clearer text-edit affordance messaging

### E. Path authoring parity

- In-panel SVG path data editing
- Improved stroke/fill/dash/trim grouping

### F. Image and masking workflow

- Media library chooser path in panel
- Visual mask/frame gallery path

### G. Group workflow completeness

- Group opacity control in group context
- Coherent clip-children + boolean op + naming composition

### H. Animation mode parity

- Keyframe property include/exclude controls
- Keyframe-routed property editing in properties panel

### I. UX polish and consistency

- Better section defaults/order by element type
- Consistent validation and helper patterns
- Strong keyboard/focus/ARIA behavior

---

## 4) File Touch Map (Allowed/Expected)

Use this map to prevent random edits.
...
### 4.2 Potential editor/model support files (only if needed)

- `packages/editor/src/**/*`
- `packages/model/src/**/*`

Only touch these when UI behavior requires new store/model plumbing. If touched, add focused unit tests in same package.

### 4.3 Required test files to add/update

- `packages/ui/src/panels.sidebar.test.tsx`
- `packages/ui/src/panels.geometry.test.tsx`
- `packages/ui/src/panels.appearance.test.tsx`
- `packages/ui/src/panels.typography.test.tsx`
- `packages/ui/src/panels.text-effects.test.tsx`
...
- `packages/ui/src/panels.group.test.tsx`
- `packages/ui/src/panels.animation-mode.test.tsx`
- `packages/demo/ct/**/*` (new/updated cross-region CTs)

---

## 5) Unit-by-Unit Ralph Plan (Strict)

Every unit has required subtasks. Do not skip subtasks.

### Unit 1. Context Header + Lock UX
...
- Header renders selected name and type chip.
- Lock control toggles state and updates label.
- Locked state disables all editable controls in visible panels.
- Locked helper text is shown.

**Implementation required**

- Implement/extend context header component.
- Wire lock toggle to store action.
- Add centralized disabled propagation behavior to panel controls.

**Acceptance criteria**

- Immediate UI feedback on lock toggle.
- No editable control remains active while locked.

### Unit 2. Geometry Anchors + Unit-Aware Editing
...
- Anchor toggle changes effective X/Y interpretation.
- Geometry labels reflect active unit.
- Unit conversion behavior is deterministic for px/mm/in.
- No regressions in auto-size and 3D controls.

**Implementation required**

- Add anchor controls and conversion plumbing.
- Add unit-aware labels and value conversions.
- Preserve existing geometry panel API compatibility.

**Acceptance criteria**

- Geometry edits map correctly to model coordinates under all anchor modes.

### Unit 3. Appearance 2.0 (Gradient + Border Radius)
...
- Fill mode switch solid/gradient works.
- Gradient stop add/remove/edit/position updates bound value.
- Gradient angle updates bound value.
- Border radius linked updates all corners.
- Border radius unlinked updates one corner only.

**Implementation required**

- Replace plain gradient string workflow with structured editor UI.
- Add border radius 4-corner editor with linked/unlinked mode.

**Acceptance criteria**

- Users can build gradients without manual CSS editing.
- Corner radius behavior is explicit and predictable.

### Unit 4. Typography + Text Effects UX Upgrade
...
- Bold/italic/underline/strikethrough toggles mutate intended properties.
- Advanced section toggle reveals and hides advanced controls.
- Helper message for text editing appears in correct contexts.

**Implementation required**

- Add fast formatting toggle cluster where applicable.
- Improve hierarchy of common vs advanced text controls.

**Acceptance criteria**

- Common tasks require fewer interactions while preserving depth.

### Unit 5. Path Panel Parity
...
- SVG path data textarea edits update rendered path.
- Draw/edit mode buttons reflect state transitions.
- Stroke/fill/dash/trim controls continue working after panel restructuring.

**Implementation required**

- Add optional path data editor area.
- Harmonize path section grouping and labels.

**Acceptance criteria**

- Path authoring supports both direct-data and control-based workflows.

### Unit 6. Image Panel Upgrade (Library + Mask Gallery)
...
- Media library action updates image source when confirmed.
- Mask gallery selection updates clipping/mask properties.
- Direct URL editing remains functional.

**Implementation required**

- Integrate chooser flow and gallery controls into image context.
- Keep power-user URL path visible.

**Acceptance criteria**

- Both novice and power-user workflows are supported.

### Unit 7. Group Panel Completeness
...
- Group opacity control updates style.
- Clip-children and boolean operation behavior remain correct.
- Group naming behavior remains correct.

**Implementation required**

- Add group opacity control with correct constraints.
- Ensure coherent group settings composition.

**Acceptance criteria**

- Group-focused editing is complete in one panel context.

### Unit 8. Animation Mode Property Editing Parity
...
- In animation mode, edits target selected keyframe properties.
- Include/exclude toggle adds/removes keyframe property entries correctly.
- Exiting animation mode restores normal element property editing path.

**Implementation required**

- Reintroduce/extend adapter pattern for keyframe routing.
- Add explicit UI state cues for animation mode.

**Acceptance criteria**

- Keyframe editing is deterministic and does not leak into base style unintentionally.

### Unit 9. Cross-Panel UX Polish and Consistency
...
- Accordion defaults and ordering vary correctly by element type.
- Validation messages are rendered consistently.
- Keyboard navigation and focus behavior pass accessibility expectations.

**Implementation required**

- Normalize field shell/help/error pattern.
- Tune default expanded sections by usage priority.

**Acceptance criteria**

- Panel scanning and editing speed improves without sacrificing clarity.

### Unit 10. Demo Integration + Regression Hardening
...
- CT: properties interaction changes canvas output.
- CT: lock state disables panel and blocks edits.
- CT: animation mode property edit mutates keyframe/timeline as expected.

**Implementation required**

- Update demo fixtures and CT scenarios.
- Ensure selectors and fixtures follow existing demo fixture policy.

**Acceptance criteria**

- Demo showcases all upgraded workflows and no key regressions.

---

## 6) Mandatory Command Gates

### Per unit (must pass before checking unit complete)

- `npm run quality -w @broadset/ui`
- `npm run quality -w @broadset/demo`

### Milestone gates (after Units 3, 8, 10)
...
Each line must have at least one CT test before final sign-off.

1. Panel lock toggle -> canvas editability/transform behavior
2. Geometry anchor/unit edits -> canvas element position
3. Gradient/radius edits -> rendered appearance change
4. Typography toggle edits -> text render style change
5. Path data edits -> path geometry update on canvas
6. Image library/mask edits -> rendered source/mask update
7. Group opacity/clip edits -> grouped render changes
8. Animation-mode edits -> timeline/keyframe state update + preview effect

---

## 8) Spec Update Rules (Additive Only)

If implementation reveals ambiguity, update docs additively only:

- `project/spec/ui/panels.md`
- `project/spec/ui/inputs.md`
- `project/implementation/component-testing.md` (if CT mapping needs clarification)

Forbidden: rewriting existing requirements to justify a shortcut.

---

## 9) PR Review Checklist

Reviewer must verify all are true:

- Unit tests added before implementation for each unit claim.
- No forbidden shortcuts present.
- HeroUI usage is compliant.
- Capability-driven visibility still correct per element type.
- Screen/print property gating preserved.
- Cross-region CT tests exist for newly introduced behaviors.
- Accessibility labels and keyboard flow are intact.
...
1. Unit 1 (context/lock)
2. Unit 2 (geometry anchors/units)
3. Unit 3 (appearance upgrade)
4. Unit 5 (path parity)
5. Unit 6 (image/mask workflow)
6. Unit 7 (group completeness)
7. Unit 4 (typography/effects polish)
8. Unit 8 (animation mode parity)
9. Unit 9 (global UX polish)
10. Unit 10 (integration + hardening)

Reasoning: establish shared panel state behavior first, then high-impact authoring workflows, then animation complexity, then stabilization.

---

## 11) Progress Tracker

| Unit | Area                         | Tests Red | Impl Green | Refactor Done | Gates Passed |
| ---- | ---------------------------- | --------- | ---------- | ------------- | ------------ |
| 1    | Context header + lock UX     | ☐         | ☐          | ☐             | ☐            |
| 2    | Geometry anchors + units     | ☐         | ☐          | ☐             | ☐            |
| 3    | Appearance 2.0               | ☐         | ☐          | ☐             | ☐            |
| 4    | Typography + text effects UX | ☐         | ☐          | ☐             | ☐            |
| 5    | Path panel parity            | ☐         | ☐          | ☐             | ☐            |
| 6    | Image workflow upgrade       | ☐         | ☐          | ☐             | ☐            |
| 7    | Group panel completeness     | ☐         | ☐          | ☐             | ☐            |
| 8    | Animation mode parity        | ☐         | ☐          | ☐             | ☐            |
| 9    | Cross-panel UX polish        | ☐         | ☐          | ☐             | ☐            |
| 10   | Demo + regression hardening  | ☐         | ☐          | ☐             | ☐            |

---

## 12) Work Log Template (Use Per Unit)

Copy this block under the unit when executing:
