...
## Baseline at plan authoring (historical — not current implementation)

**This section describes the pre-track `@libpdf/core` exporter. Current implementation uses `pdf-lib` + `pdfjs-dist` on `dev2-phase-11`. See [plan-progress.md](./plan-progress.md) for tiered status.**

**Broadset at plan authoring — [packages/formats/src/pdf/](../../packages/formats/src/pdf/):**
