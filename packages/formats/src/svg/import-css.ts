...
function processRuleNode(
  ...
  for (const selectorNode of selectorListChildren(rule.prelude)) {
    if (out.length >= budget) {
      reachedBudget = true;
      break;
    }
...
function applyStylePresentation(el: Element, body: string): void {
  ...
  for (const [prop, value] of resolvedPaintEntries(resolved)) {
    el.setAttribute(prop, value);
  }
}

function selectorListChildren(selectorList: CssTree.SelectorList): readonly CssTree.CssNode[] {
  return selectorList.children.toArray();
}

function resolvedPaintEntries(resolved: ReadonlyMap<string, string>): readonly (readonly [string, string])[] {
