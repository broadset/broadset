...
export interface PsdExportOptions {
  ...
  /** Async URL-image fetch timeout in milliseconds. Defaults to the shared safe-fetch timeout. */
  readonly imageFetchTimeoutMs?: number | undefined;
  /** Async URL-image byte cap. Defaults to the shared safe-fetch max byte budget. */
  readonly maxImageBytes?: number | undefined;
  /** Optional async URL-image host allowlist. Values match `URL.host`, e.g. `assets.example.com`. */
  readonly allowedImageHosts?: ReadonlySet<string> | undefined;
}
