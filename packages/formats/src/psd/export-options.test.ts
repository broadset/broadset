...
describe('PSD export — PsdExportOptions plumbing', () => {
  ...
  /**
   * @description Requesting `linkSmartObjects: false` MUST surface a
   * preflight warning that explains the ripple is not yet honoured;
   * smart objects continue to ship as linked.
   */
  it('warns when linkSmartObjects is false', async () => {
    const el = makeElement('rectangle', { id: 'r1', name: 'Rect' });
    const doc = makeDocument({ elements: [el] });
    const result = await exportPsdBytesAsyncWithPreflight(doc, { linkSmartObjects: false });

    expect(result.bytes.length).toBeGreaterThan(0);
    expect(result.warnings.some((w) => w.toLowerCase().includes('linksmartobjects'))).toBe(true);
  });
