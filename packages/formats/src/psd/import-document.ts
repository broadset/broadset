import { createEmptyBroadsetDocument, ensureRootElementsHavePageInstances } from '@broadset/model';
...
function formatParserErrorDetail(error: unknown): string | null {
...
function buildMalformedPsdWarning(error: unknown): string {
  const detail = formatParserErrorDetail(error);

  if (!detail) return MALFORMED_PSD_WARNING;

  return `${MALFORMED_PSD_WARNING} Parser detail: ${detail}`;
}
...
export function importPsdDocument(data: Uint8Array, options?: PsdImportOptions): DocumentImportResult {
  ...
  try {
    ...
    return { document: createEmptyBroadsetDocument(), warnings: [buildMalformedPsdWarning(error)] };
  }
