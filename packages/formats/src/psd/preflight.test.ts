...
describe('exportPsdBytesAsyncWithPreflight', () => {
  ...
  /**
   * @description Async URL-image fetching MUST honor the caller's
   * byte cap and surface the violated cap in the warning instead of
   * silently falling back to placeholder pixels.
   */
  it('honors maxImageBytes for async URL image fetches', async () => {
    const el = makeElement('image', { id: 'i1', content: 'https://example.test/too-large.png', name: 'HugeImage' });
    const doc = makeDocument({ elements: [el] });
    const result = await exportPsdBytesAsyncWithPreflight(doc, {
      fetch: () => Promise.resolve(new Response(new Uint8Array(2048), { status: 200 })),
      maxImageBytes: 1024,
    });

    expect(result.bytes.length).toBeGreaterThan(0);
    expect(result.warnings.some((w) => /HugeImage.*max-bytes-exceeded.*1024/i.test(w))).toBe(true);
  });
});
