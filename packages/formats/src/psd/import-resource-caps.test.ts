...
describe('PSD importer — parser-boundary caps', () => {
  ...
  /**
   * @description A layer that has bounds but no text, pixels, vector
   * mask, placed-layer data, or children can only be represented as a
   * generic Broadset placeholder. IO-D-18 requires a visible warning
   * for that lossy fallback instead of silently pretending fidelity.
   */
  it('emits a warning when an unmapped layer is imported as a placeholder rectangle', () => {
    const psdBytes = writePsdUint8Array({
      width: 32,
      height: 32,
      channels: 4,
      ...
      children: [
        {
          name: 'Adjustment-like empty layer',
          left: 4,
          top: 4,
          right: 20,
          bottom: 20,
    ...
    const result = importPsdDocument(psdBytes);

    expect(result.document.elements[0]?.type).toBe('rectangle');
    expect(result.warnings.some((w) => /mapped as placeholder rectangle/i.test(w))).toBe(true);
  });
});
