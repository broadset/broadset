...
async function fetchImageAsBytes(
  ...
  options: PsdExportOptions | undefined,
): Promise<PsdImageFetchResult> {
  ...
  return { ok: true, mime, bytes: result.bytes };
}

function formatImageFetchFailure(result: Extract<PsdImageFetchResult, { readonly ok: false }>): string {
  const details = [
    result.reason,
    result.status === undefined ? undefined : `status ${String(result.status)}`,
    result.cap === undefined ? undefined : `cap ${String(result.cap)}`,
    result.detail,
...
export async function exportPsdBytesAsync(doc: BroadsetDocument, options?: ExportPsdAsyncOptions): Promise<Uint8Array> {
  ...
  const fetchResults = await Promise.all(
    urlElements.map(async (el) => {
      const result = await fetchImageAsBytes(resolveContentAsPlainString(el.content), fetchFn, options);
  ...
  for (const { id, result } of fetchResults) {
    if (result.ok) {
      prefetched.set(id, { mime: result.mime, bytes: result.bytes });
    }
  }
...
export async function exportPsdBytesAsyncWithPreflight(
  ...
  const fetchResults = await Promise.all(
    urlElements.map(async (el) => {
      ...
      const result = await fetchImageAsBytes(url, fetchFn, options);
  ...
  for (const { id, name, url, result } of fetchResults) {
    ...
    fetchWarnings.push(
      `PSD preflight: failed to fetch image "${name}" from ${url} (${formatImageFetchFailure(result)}); the layer will export with placeholder pixels.`,
    );
  }
