...
describe('sanitizeSvg', () => {
  ...
  /**
   * @description Namespaced event-handler attributes
   * (`xlink:onclick`, `ev:onload`) must be stripped in the shared
   * sanitizer too, because opaque `svg` elements are sanitized by this
   * path during export rather than by the SVG importer DOM walk.
   */
  it('strips namespaced event-handler attributes', () => {
    const result = sanitizeSvg(
      '<svg xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:ev="http://www.w3.org/2001/xml-events"><rect xlink:onclick="x()" ev:onload="y()"/></svg>',
    );

    expect(result.ast.markup).not.toContain('onclick');
    expect(result.ast.markup).not.toContain('onload');
    expect(result.ast.markup).not.toContain('x()');
    expect(result.ast.markup).not.toContain('y()');
  });
  ...
  /**
   * @description Non-image data URLs and local-file URLs are active or
   * local-read surfaces in downstream SVG viewers. The shared
   * sanitizer must reject them while preserving safe image data URLs.
   */
  it('strips unsafe URL schemes but keeps data:image hrefs', () => {
    const result = sanitizeSvg(
      '<svg><image href="file:///etc/passwd"/><image href="data:text/html;base64,PHNjcmlwdD4="/><image href="data:image/png;base64,iVBORw0KGgo="/></svg>',
    );

    expect(result.ast.markup).not.toContain('file:///');
    expect(result.ast.markup).not.toContain('data:text/html');
    expect(result.ast.markup).toContain('data:image/png');
  });
