...
function stripUnsafeUrlAttrs(markup: string): {
  readonly markup: string;
  readonly removed: readonly SvgSanitizationRemoval[];
} {
  try {
    ...
    const removed: SvgSanitizationRemoval[] = [];
    const all = Array.from(root.getElementsByTagName('*'));

    for (const el of [root, ...all]) {
      for (const attr of URL_ATTRS_TO_CHECK) {
        const value = el.getAttribute(attr);

        if (value !== null && !isAllowedSvgUrl(value.trim())) {
          el.removeAttribute(attr);
          removed.push({ kind: 'attribute', name: attr.toLowerCase(), from: el.tagName.toLowerCase() });
        }
      }
    }
...
export function sanitizeSvg(input: string): SvgSanitizationResult {
  ...
  const urlSanitized = stripUnsafeUrlAttrs(sanitizedWrapped);
  const removed = [...collectRemovals(), ...urlSanitized.removed];
  const markup = unwrapFragmentMarkup(urlSanitized.markup, isFragment);
