...
export async function buildPptxPackageWithReport(
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    for (const [mediaPath, mediaBytes] of mediaEntries(slideXml.media)) {
...
const MEDIA_CONTENT_TYPES_BY_EXTENSION: ReadonlyMap<
  string,
  { readonly extension: string; readonly contentType: string }
> = new Map([
...
function mediaEntries(media: ReadonlyMap<string, Uint8Array>): readonly (readonly [string, Uint8Array])[] {
  return Array.from(media.entries());
}
...
function attachNotesMasterIfNeeded(options: {
  ...
  const hasAnyNotes = pages.some(pageHasNotes);
  ...
  parts.set(
    ...
    encodeText(buildRelationshipsXml(notesMasterRels.entries())),
  );
...
function pageHasNotes(page: Page): boolean {
  return typeof page.notes === 'string' && page.notes.length > 0;
}
...
function attachNotesForSlide(options: {
  ...
  const slideNotesRels = [
    ...slideRelsEntries,
    {
      id: `rId${String(slideRelsEntries.length + 1)}` as `rId${number}`,
      type: OOXML_REL_TYPES.notesSlide,
      target: `../notesSlides/${notesFile}`,
    },
  ];
 ...
 */
function applyOverrideToElement(element: BroadsetElement, override: Page['elements'][number]): BroadsetElement {
...
function buildPresentationXml(document: BroadsetDocument, slideCount: number, embeddedFontLst = ''): string {
  ...
  const slideIds = Array.from(
    { length: slideCount },
    (_, i) => `<p:sldId id="${String(256 + i)}" r:id="rId${String(2 + i)}"/>`,
  ).join('');
