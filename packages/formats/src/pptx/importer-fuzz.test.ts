...
function buildVbaPackage(): Uint8Array {
  return buildPackageWith([
    ['ppt/vbaProject.bin', new Uint8Array([0xd0, 0xcf, 0x11, 0xe0])],
  ]);
}

function buildOleEmbeddingPackage(): Uint8Array {
  return buildPackageWith([
    ['ppt/embeddings/oleObject1.bin', new Uint8Array(1024)],
  ]);
}
  ...
  for (const fuzzCase of FUZZ_CASES) {
    it(`survives without throwing: ${fuzzCase.name}`, () => {
      ...
      if (fuzzCase.expectsWarning !== undefined) {
        ...
        expect(matched, `expected a warning matching ${String(fuzzCase.expectsWarning)} for ${fuzzCase.name}; got ${JSON.stringify(result.warnings)}`).toBe(true);
      }
    });
  }
});
