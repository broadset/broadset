...
describe('validatePptxPackage — malformed packages', () => {
  ...
  it('reports capped entries without inflating them during validation', () => {
    const bytes = writeOoxmlPackage(
      new Map([
        ['[Content_Types].xml', encodeText('<Types/>')],
        [
        ...
        ['ppt/slides/oversized.xml', new Uint8Array(2 * 1024 * 1024)],
      ]),
    );
    const result = validatePptxPackage(bytes, { maxPartBytes: 1024 });

    expect(result.valid).toBe(false);
    expect(result.issues.some((i) => i.code === 'size-cap' && i.part === 'ppt/slides/oversized.xml')).toBe(true);
  });
});
