...
type PptxValidationOptions = PptxImportOptions;

/**
 * Run validation against a PPTX package.
 */
export function validatePptxPackage(bytes: Uint8Array, options?: PptxValidationOptions): ValidationResult {
  const issues: ValidationIssue[] = [];

  const pkg = tryRead(bytes, issues, options);
...
function tryRead(bytes: Uint8Array, issues: ValidationIssue[], options?: PptxValidationOptions): OoxmlPackage | null {
  const caps = resolvePptxImportCaps(options);

  if (bytes.byteLength > caps.maxInputBytes) {
    issues.push({
      level: 'error',
      code: 'size-cap',
      message: `Input size ${String(bytes.byteLength)} exceeds cap ${String(caps.maxInputBytes)} bytes`,
    });
  ...
  try {
    ...
    for (const skipped of result.skippedEntries) {
      ...
      const issue = {
        ...
        ...(warning.detail === undefined ? {} : { part: warning.detail }),
...
function validateXmlParsability(pkg: OoxmlPackage, issues: ValidationIssue[]): void {
...
function validateMacroRejection(pkg: OoxmlPackage, issues: ValidationIssue[]): void {
