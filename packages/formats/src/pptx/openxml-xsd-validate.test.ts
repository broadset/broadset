...
describe('ECMA-376 / ISO/IEC 29500 schema validation', () => {
  if (!REQUIRE_OPENXML_VALIDATOR) {
    it.skip('set BROADSET_REQUIRE_OPENXML_VALIDATOR=1 to run the Open XML SDK validator', () => {
      expect(REQUIRE_OPENXML_VALIDATOR).toBe(false);
    });
  ...
  it(
    ...
    () => {
      ...
      const result = spawnSync(
        dotnet,
        ['run', '--project', projectPath, '--configuration', 'Release', '--', pptxPath],
        {
          encoding: 'utf8',
