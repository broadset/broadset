...
export function extractThirdPartyElementsWithBudget(
  ...
  return {
    ...
    capExceeded: extraction.capExceeded,
    capBytes: extraction.capBytes,
  };
}
...
function toTextElement(item: ExtractedTextItem, canvas: Canvas, heightPt: number, index: number): BroadsetElement {
  const lineHeightPt = Math.max(item.fontSizePt * APPROX_LINE_HEIGHT_MULTIPLIER, MIN_LINE_HEIGHT_PT);
  const approxWidthPt = Math.max(item.text.length * item.fontSizePt * APPROX_CHAR_WIDTH_RATIO, MIN_ELEMENT_SIDE_PT);
