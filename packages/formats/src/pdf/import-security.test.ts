...
async function buildPdfWithCatalogEntry(apply: (pdf: PDFDocument) => void | Promise<void>): Promise<Uint8Array> {
...
describe('P6.4 security — encryption rejection', () => {
  ...
  /**
   * @description Supplying a password option must not cause generic
   * malformed bytes to be reported as encrypted. That distinction is
   * important for user recovery: a corrupted file cannot be fixed by
   * retrying with another password.
   */
  it('does not conflate malformed input with encrypted input when a password option is present', async () => {
    const malformed = new TextEncoder().encode('%PDF-1.7\nnot a real xref\n%%EOF');
    const result = await importPdfDocument(malformed, { password: 'secret' });

    expect(result.warnings.some((w) => w.toLowerCase().includes('malformed'))).toBe(true);
    expect(result.warnings.some((w) => w.toLowerCase().includes('encrypted'))).toBe(false);
  });
});
    ...
    const bytes = await buildPdfWithCatalogEntry((pdf) => {
      const namedActions = pdf.context.obj({
        Names: pdf.context.obj([
          PDFString.of('malicious-action'),
          pdf.context.obj({ S: 'JavaScript', JS: PDFString.of('app.alert("hi")') }),
        ]),
      });
...
describe('P6.4 security — embedded-file preservation', () => {
  ...
  it('preserves embedded-file names on extensions.pdf.embeddedFiles and warns', async () => {
    ...
    expect(embeddedWarning?.toLowerCase()).toContain('names preserved');
    expect(embeddedWarning?.toLowerCase()).toContain('bytes dropped');
