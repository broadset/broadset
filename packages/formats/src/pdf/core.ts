...
function resolveRectangleStyle(el: BroadsetElement, canvas: Canvas): RectangleStyleResolved {
  return {
    ...
    borderWidthPt: el.style.borderWidth !== undefined ? elementToPoints(canvas, el.style.borderWidth) : undefined,
...
async function renderElement(
  ...
  if (ocgBinding !== undefined) {
    page.pushOperators(
      PDFOperator.of(PDFOperatorNames.BeginMarkedContentSequence, [PDFName.of('OC'), ocgBinding.resourceName]),
    );
  }
...
async function runExport(
  ...
  const xmpPdfa =
    pdfaConformance !== undefined ? { part: '2', conformance: pdfaConformanceLetter(pdfaConformance) } : undefined;
