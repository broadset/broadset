...
//   - every committed binary fixture under
//     `packages/formats/src/pdf/__fixtures__/verapdf/` whose name encodes
...
const FIXTURE_DIR = resolve(FORMATS_PKG_DIR, 'src/pdf/__fixtures__/verapdf');
...
async function main() {
  ...
  // Build the fixture target list: every committed verapdf/*.pdf.
  let entries = [];

  try {
    entries = await readdir(FIXTURE_DIR);
  } catch {
    logError(`fixture directory not found: ${FIXTURE_DIR}`);
    process.exit(1);
  }
  ...
  const targets = entries
    ...
    .map((name) => ({
      ...
      label: `verapdf/${name}`,
      expected: name.includes('-pass') ? 'compliant' : name.includes('-fail') ? 'non-compliant' : 'compliant',
    }));

  // Plus a fresh Broadset PDF/A-2b export.
  const tmp = await mkdtemp(join(tmpdir(), 'broadset-verapdf-'));
...
await mkdir(dirname(FIXTURE_DIR), { recursive: true });
main().catch((err) => {
