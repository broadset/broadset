...
describe('AppearancePanel', () => {
  ...
  /** @description Switching from Solid to Gradient with no prior draft must emit a valid default gradient, not an empty sentinel reserved for clearing gradients. */
  it('emits a default gradient when switching from solid fill to gradient fill', () => {
    const onUpdate = vi.fn<(key: string, value: PropertyValue) => void>();

    render(<AppearancePanel {...BASE_PROPS} backgroundGradient="" showGradient onUpdate={onUpdate} />);

    fireEvent.click(screen.getByRole('button', { name: 'Gradient' }));

    expect(onUpdate).toHaveBeenCalledWith('backgroundGradient', 'linear-gradient(90deg, #ff0000 0%, #0000ff 100%)');
  });

  /** @description Fill mode must follow the selected keyframe value when animation selection changes between solid and gradient keyframes. */
  it('syncs fill mode when the backgroundGradient prop changes after mount', () => {
    const { rerender } = render(
      <AppearancePanel {...BASE_PROPS} backgroundGradient="" showGradient onUpdate={() => undefined} />,
    );

    expect(screen.queryByRole('group', { name: 'Gradient stops' })).toBeNull();
