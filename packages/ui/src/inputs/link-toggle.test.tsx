...
describe('LinkToggle', () => {
  ...
  it('exposes aria-pressed reflecting the linked state', () => {
    const { rerender } = render(<LinkToggle isLinked={false} onToggle={jest.fn()} ariaLabel="Link corners" />);

    expect(screen.getByRole('button', { name: 'Link corners' }).getAttribute('aria-pressed')).toBe('false');

    rerender(<LinkToggle isLinked onToggle={jest.fn()} ariaLabel="Link corners" />);
  ...
  it('emits inverted state on press', () => {
    const onToggle = jest.fn<(next: boolean) => void>();
