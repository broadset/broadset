...
import { usePropertyEditingAdapter } from './property-editing-context-core';
...
const DEFAULT_LINEAR_GRADIENT = 'linear-gradient(90deg, #ff0000 0%, #0000ff 100%)';
...
type RotationZKey = 'rotation' | 'rotateZ';

function hasGeometry3DValues({
  rotateX,
  rotateY,
  rotateZ,
  translateZ,
}: Pick<GeometryPanelProps, 'rotateX' | 'rotateY' | 'rotateZ' | 'translateZ'>): boolean {
  return rotateX !== undefined || rotateY !== undefined || rotateZ !== undefined || translateZ !== undefined;
}

function resolveRotationZKey(has3DValues: boolean, rotateZ: number | undefined): RotationZKey {
  return has3DValues && rotateZ !== undefined ? 'rotateZ' : 'rotation';
}

function getPositionRelatedPropertyKeys(showZAxes: boolean): readonly string[] {
  return showZAxes ? ['y', 'translateZ'] : ['y'];
}

function getPositionRelatedDefaultValues(
  showZAxes: boolean,
  y: number,
  translateZ: number | undefined,
): Readonly<Record<string, number>> {
  return showZAxes ? { y, translateZ: translateZ ?? 0 } : { y };
}

function getRotationRelatedPropertyKeys(showZAxes: boolean): readonly string[] {
  return showZAxes ? ['rotateX', 'rotateY'] : [];
}
...
export function GeometryPanel({
  ...
  const propertyAdapter = usePropertyEditingAdapter();
  const isPropertyEditable = useCallback(
    (propertyKey: string): boolean => propertyAdapter === null || propertyAdapter.isIncluded(propertyKey),
    [propertyAdapter],
  );
  ...
  const has3DValues = hasGeometry3DValues({ rotateX, rotateY, rotateZ, translateZ });
  ...
  const rotationZKey = resolveRotationZKey(has3DValues, rotateZ);
  const rotationZValue = rotationZKey === 'rotateZ' ? (rotateZ ?? 0) : rotation;
  const positionRelatedPropertyKeys = getPositionRelatedPropertyKeys(showZAxes);
  const positionRelatedDefaultValues = getPositionRelatedDefaultValues(showZAxes, y, translateZ);
  const rotationRelatedPropertyKeys = getRotationRelatedPropertyKeys(showZAxes);
  const rotationRelatedDefaultValues = { rotateX: rotateX ?? 0, rotateY: rotateY ?? 0 };
    ...
    () => [
      {
        ...
        value: displayX,
        isDisabled: !isPropertyEditable('x'),
      ...
      },
      {
        ...
        value: displayY,
        isDisabled: !isPropertyEditable('y'),
      ...
      },
      {
        ...
        value: translateZ ?? 0,
        isDisabled: !isPropertyEditable('translateZ'),
    ...
    () => [
      {
        ...
        value: rotateX ?? 0,
        isDisabled: !isPropertyEditable('rotateX'),
      ...
      },
      {
        ...
        value: rotateY ?? 0,
        isDisabled: !isPropertyEditable('rotateY'),
      ...
      },
      {
        ...
        isDisabled: !isPropertyEditable(rotationZKey),
        onChange: (nextRotZ: number) => {
          onUpdate(rotationZKey, nextRotZ);
        },
      },
    ],
    [isPropertyEditable, onUpdate, rotateX, rotateY, rotationZKey, rotationZValue, showZAxes],
  );
  ...
  return (
    ...
    >
      <PropertyField
        propertyKey="x"
        relatedPropertyKeys={positionRelatedPropertyKeys}
        defaultValue={x}
        relatedDefaultValues={positionRelatedDefaultValues}
      >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: sp('sp-02'), minWidth: 0, width: '100%' }}>
        <PropertyField
          propertyKey="width"
          relatedPropertyKeys={['height']}
          defaultValue={width}
          relatedDefaultValues={{ height }}
        >
          <PairInput
            label="Size"
            unit={documentUnit}
            axes={[
              {
                ...
                value: displayWidth,
                min: 0.1,
                isDisabled: !isPropertyEditable('width'),
              ...
              },
              {
                ...
                value: displayHeight,
                min: 0.1,
                isDisabled: isAutoHeight || !isPropertyEditable('height'),
      ...
      <PropertyField
        propertyKey={rotationZKey}
        relatedPropertyKeys={rotationRelatedPropertyKeys}
        defaultValue={rotationZValue}
        relatedDefaultValues={rotationRelatedDefaultValues}
      >
...
export function AppearancePanel({
  ...
  useEffect(() => {
    const nextGradient = backgroundGradient?.trim() ?? '';
    ...
    if (nextGradient !== '') {
      ...
      setDraftGradient(backgroundGradient ?? nextGradient);

      return;
    }

    setFillMode('solid');
  }, [backgroundGradient, showGradient]);
            ...
            <SegmentedSwitcher
              ...
              onChange={(nextFillMode) => {
                ...
                if (nextFillMode === 'gradient') {
                  const nextGradient = draftGradient.trim() === '' ? DEFAULT_LINEAR_GRADIENT : draftGradient;
