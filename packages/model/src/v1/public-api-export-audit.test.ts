...
describe('v1 compiler export inventory', () => {
  it('matches every value and type symbol against fixed public API oracles', () => {
    ...
    expect(V1_COMPLETE_EXPORT_NAMES).toHaveLength(327);
    expect(V1_RUNTIME_EXPORT_NAMES).toHaveLength(143);
    expect(V1_TYPE_EXPORT_NAMES).toHaveLength(185);
  ...
  }, 60000); // builds and type-checks a full TS program; needs headroom beyond the 5s default under parallel load
});
