...
function collectPageElements(args: {
  ...
  const appendSubtree = (element: BroadsetElement): void => {
    const instance = instanceMap.get(element.id);

    if (element.parentId !== null && instance?.visible === false) {
      return;
    }

    ordered.push(
      instance === undefined ? element : (
        {
          ...element,
          position: {
          ...
          width: element.width * instance.transform.scale.x,
          height: element.height * instance.transform.scale.y,
        }
      ),
    );
...
export function buildLayerInfoList(document: BroadsetDocument, activePageIndex: number): readonly LayerInfo[] {
  ...
  const getLayerVisible = (element: BroadsetElement): boolean => {
    const directInstance = pageInstanceById.get(element.id);

    if (directInstance !== undefined) {
      return directInstance.visible;
    }
...
export function toPanelElement(element: BroadsetElement, instance?: PageElementInstance): PanelElement {
  ...
  return {
    ...
    stroke: resolveStyleColor(element.style.stroke, { resolveTheme: false }) ?? '',
