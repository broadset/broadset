...
describe('DemoApp chrome and menu integration', () => {
  ...
  /** @description Parented element selections must mirror the renderer hierarchy so the widget inherits parent transforms before applying the child's local geometry. */
  it('wraps parented transform widgets in ancestor transform hosts', () => {
    setupDemoShellMocks();
    ...
    const ancestorElements: BroadsetElement[] = [];
    let parent = elementById.get(parentedElement.parentId ?? '');

    while (parent !== undefined) {
      ancestorElements.push(parent);
    ...
    const widget = screen.getByTestId('demo-transform-widget');
    const widgetStyle = widget.getAttribute('style') ?? '';

    expect(widgetStyle).toContain(`left: ${String(parentedElement.position.x)}px`);
    expect(widgetStyle).toContain(`top: ${String(parentedElement.position.y)}px`);

    let ancestorWrapper = widget.parentElement?.parentElement?.parentElement;

    for (const ancestorElement of ancestorElements) {
      const ancestorStyle = ancestorWrapper?.getAttribute('style') ?? '';

      expect(ancestorStyle).toContain(`left: ${String(ancestorElement.position.x)}px`);
      expect(ancestorStyle).toContain(`top: ${String(ancestorElement.position.y)}px`);

      ancestorWrapper = ancestorWrapper?.parentElement?.parentElement?.parentElement;
    }
  });
