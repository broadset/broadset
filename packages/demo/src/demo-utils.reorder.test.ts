...
describe('toPanelElement', () => {
  /** @description The Appearance panel consumes CSS gradient strings, not serialized structured JSON; otherwise the gradient editor falls back to default stops after every model update. */
  it('serializes structured gradient fills as CSS gradient strings for the panel adapter', () => {
    ...
    const panelElement = toPanelElement(element);

    expect(panelElement.backgroundGradient).toBe('linear-gradient(45deg, #112233 0%, #445566 100%)');
  });
});
...
describe('buildLayerInfoList', () => {
  ...
  /** @description Nested layer rows must report their own page visibility override when present, not only inherit the root parent's visibility. */
  it('marks child layers hidden when the active page has a child visibility override', () => {
    const base = createEmptyBroadsetDocument();
    const parent = createDefaultElement('group', { id: 'el-parent', name: 'Parent' });
    const child = createDefaultElement('text', { id: 'el-child', name: 'Child', parentId: parent.id });
    ...
    const document: BroadsetDocument = {
      ...
      pages: base.pages.map((page) => ({
        ...page,
        elements: [
          ...createRootPageInstances(elements),
          {
            elementId: child.id,
            transform: {
              position: { x: child.position.x, y: child.position.y, z: 0 },
              rotation: { x: 0, y: 0, z: child.rotation },
              scale: { x: 1, y: 1, z: 1 },
            },
            visible: false,
          },
    ...
    expect(layers.find((layer) => layer.id === parent.id)?.visible).toBe(true);
    expect(layers.find((layer) => layer.id === child.id)?.visible).toBe(false);
  });
});
...
describe('buildRenderableDocumentForActivePage', () => {
  ...
  /** @description Active-page child visibility overrides must remove hidden descendants from the renderable document while leaving the visible parent and siblings intact. */
  it('omits child elements hidden by active-page visibility overrides', () => {
    const base = createEmptyBroadsetDocument();
    const parent = createDefaultElement('group', { id: 'el-parent', name: 'Parent' });
    const hiddenChild = createDefaultElement('text', {
      id: 'el-hidden-child',
      name: 'Hidden Child',
      parentId: parent.id,
    });
    const visibleChild = createDefaultElement('text', {
      id: 'el-visible-child',
      name: 'Visible Child',
      parentId: parent.id,
    });
    const elements = [parent, hiddenChild, visibleChild];
    const document: BroadsetDocument = {
      ...base,
      elements,
      pages: base.pages.map((page) => ({
        ...
        elements: [
          ...createRootPageInstances(elements),
          {
            elementId: hiddenChild.id,
            transform: {
              position: { x: hiddenChild.position.x, y: hiddenChild.position.y, z: 0 },
              rotation: { x: 0, y: 0, z: hiddenChild.rotation },
              scale: { x: 1, y: 1, z: 1 },
            },
            visible: false,
          },
    ...
    expect(renderable.elements.map((element) => element.id)).toEqual([parent.id, visibleChild.id]);
  });
});
