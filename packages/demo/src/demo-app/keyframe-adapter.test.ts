...
describe('createKeyframeAdapter', () => {
  ...
  /** @description Keyframe writes must use the editor animation-config action rather than mutating the document through raw store.setState, so animation preview effects can distinguish timeline edits from base document edits. */
  it('routes keyframe writes through the animation config store action', () => {
    const updateConfigSpy = vi.spyOn(store.getState(), 'updateElementAnimationConfig');
    const setStateSpy = vi.spyOn(store, 'setState');
    const adapter = createKeyframeAdapter(store, ELEMENT_ID, 'tl-1', 0, onTimelineUpdated);

    adapter.toggleProperty('y', true, 42);

    expect(updateConfigSpy).toHaveBeenCalledWith(ELEMENT_ID, expect.any(Function));
  ...
  /** @description Gradient CSS strings are structured as string keyframe values, not color values, so the animation panel can edit gradient fills. */
  it('creates string-typed value for gradient CSS defaults', () => {
    const adapter = createKeyframeAdapter(store, ELEMENT_ID, 'tl-1', 0, onTimelineUpdated);

    adapter.toggleProperty('backgroundGradient', true, 'linear-gradient(#fff, #000)');

    const animation = store.getState().document.animations.find((entry) => entry.elementId === ELEMENT_ID);
    const property = animation?.config.timelines[0]?.keyframes[0]?.properties['backgroundGradient'];
