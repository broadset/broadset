...
import { Chip, Separator, Toolbar } from '@heroui/react';
...
function getPlaybackToggleLabel(args: {
  readonly editingTimelineName: string | null;
  readonly isPlaybackActive: boolean;
}): string {
  if (args.editingTimelineName === null) {
    return args.isPlaybackActive ? 'Pause playback' : 'Play playback';
  }

  if (args.isPlaybackActive) {
    return `Pause timeline ${args.editingTimelineName}`;
  }

  return `Play timeline ${args.editingTimelineName}`;
}

function getPlaybackResetLabel(editingTimelineName: string | null): string {
  if (editingTimelineName === null) {
    return 'Reset playback';
  }

  return `Reset timeline ${editingTimelineName}`;
}

function getTimelineStatusLabel(timelineName: string | null, isPlaying: boolean): string | null {
  if (timelineName === null) {
    return null;
  }

  return `${timelineName} · ${isPlaying ? 'Playing' : 'Paused'}`;
}
...
export function LayoutCanvasChrome(props: DemoAppLayoutProps): React.JSX.Element {
  ...
  const editingTimelineName = editingTimeline?.name ?? null;
  const timelineStatusLabel = getTimelineStatusLabel(editingTimelineName, isTimelinePreviewPlaying);
  const playbackToggleLabel = getPlaybackToggleLabel({ editingTimelineName, isPlaybackActive });
  const playbackResetLabel = getPlaybackResetLabel(editingTimelineName);
    ...
    >
      ...
      >
        {timelineStatusLabel !== null ?
          <Chip size="sm">{timelineStatusLabel}</Chip>
        : null}
        <IconToolButton
