...
import { type BooleanOperation, type BroadsetElement, gradientFill, rgbColor, solidFill } from '@broadset/model';
...
const PROPERTY_UPDATE_HANDLERS: Readonly<Record<string, (ctx: PropertyUpdateContext) => void>> = {
  ...
  backgroundColor: ({ store, element, value }) => {
    try {
      store.getState().updateElementStyle(element.id, { fill: solidFill(rgbColor(String(value))) });
    } catch {
      return;
    }
  },
  backgroundGradient: ({ store, element, value }) => {
    try {
      if (String(value).trim() === '') {
        store.getState().updateElementStyle(element.id, {
          fill: solidFill(resolveSolidFillColorForGradientClear(element)),
        });

        return;
      }

      const gradient = parseCssGradient(String(value));

      if (gradient === null) {
        return;
      }

      store.getState().updateElementStyle(element.id, { fill: gradientFill(gradient) });
    } catch {
      return;
    }
  },
