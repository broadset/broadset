import { createLogger, type LogErrorOptions, type Logger, type LogOptions } from 'vite';
 ...
 * `plugin-react-oxc` requires Vite ^6.3.0, so it would technically
 * work, but we cannot pass `disableOxcRecommendation: true` to the
 * plugin instance because Playwright's `frameworkPluginFactory`
 * constructs it with no arguments and the framework plugin is wired
 * before any user `ctViteConfig.plugins` entry can intervene.
 *
