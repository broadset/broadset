...
export function togglePageElementVisibility(
  ...
  elementId: string,
  documentElements: readonly BroadsetElement[],
): readonly PageElementInstance[] {
  const elementIndex = pageElements.findIndex((element) => element.elementId === elementId);

  if (elementIndex === -1) {
    const documentElement = documentElements.find((element) => element.id === elementId);
    ...
    return [
      ...pageElements,
      {
        elementId,
        transform: {
          position: { x: documentElement.position.x, y: documentElement.position.y, z: 0 },
          rotation: { x: 0, y: 0, z: documentElement.rotation },
          scale: { x: 1, y: 1, z: 1 },
        },
        visible: false,
      },
