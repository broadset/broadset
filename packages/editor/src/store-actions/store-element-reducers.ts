...
export function computeRemoveElementState(
  ...
  const affectedIdList = setValues(affectedIds);
  const promotedIds = new Set<string>(
    affectedIdList.filter((affectedId) => affectedId !== elementId && requiredElementIds.has(affectedId)),
  );
  const deletedIds = new Set<string>(affectedIdList.filter((affectedId) => !promotedIds.has(affectedId)));
...
export function computeRemoveElementsState(
  ...
  for (const elementId of removableIds) {
    ...
    for (const descendantId of setValues(descendants)) {
