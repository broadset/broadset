...
function isNumberArray(value: unknown): value is readonly number[] {
  ...
  return items.every(isNumberValue);
}
...
function keyframePropertyKeys(options: InterpolateKeyframePropertiesOptions): readonly string[] {
  return Array.from(new Set<string>([...Object.keys(options.fromProperties), ...Object.keys(options.toProperties)]));
}
...
export function interpolateKeyframeProperties(
  ...
  for (const key of keyframePropertyKeys(options)) {
    if (key === 'pathCommands') {
      continue;
    }
