...
describe('computeTimelineFrame', () => {
  ...
  /** @description Targeted action keyframes must update the target element state, not the owner timeline state. */
  it('routes targeted action state to target timeline state', () => {
    const timeline = createTimeline({
      id: 'targeted-action',
      name: 'Targeted Action',
      keyframes: [
        createKeyframe({ name: 'owner-start', offsetMs: 0, action: 'setState', payload: 'OWNER' }),
        createKeyframe({
          name: 'target-state',
          offsetMs: 100,
          action: 'setState',
    ...
    const frame = computeTimelineFrame({ timeline, timeMs: 250 });

    expect(frame.activeState).toBe('OWNER');
    expect(Array.from(frame.modifiers)).toEqual([]);
    expect(frame.targetStates?.['child-el']?.activeState).toBe('TARGET');
    expect(Array.from(frame.targetStates?.['child-el']?.modifiers ?? [])).toEqual(['glow']);
  });
