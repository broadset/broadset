...
export class DOMScreenRenderer implements ScreenRendererController {
  ...
  constructor(options: ScreenRendererOptions) {
    ...
    this.canvasTransformLayer.style.pointerEvents = 'none';
    ...
    this.canvasRoot.style.pointerEvents = 'none';
    this.canvasRoot.style.transformStyle = 'preserve-3d';
    this.canvasRoot.style.zIndex = '0';
    ...
    this.elementLayer.style.pointerEvents = 'none';
    ...
    this.overlayRoot.style.zIndex = '1';
  ...
  destroy(): void {
    ...
    for (const record of rendererRecordValues(this.rendererRecords)) {
  ...
  private computeChangeKinds(
    ...
    for (const [previousId, previousElement] of elementEntries(this.previousElementsById)) {
  ...
  private destroyRemovedRecords(activeIds: ReadonlySet<string>): void {
    for (const [elementId, record] of rendererRecordEntries(this.rendererRecords)) {
...
function rendererRecordValues(records: ReadonlyMap<string, RendererRecord>): readonly RendererRecord[] {
  return Array.from(records.values());
}

function rendererRecordEntries(
  records: ReadonlyMap<string, RendererRecord>,
): readonly (readonly [string, RendererRecord])[] {
  return Array.from(records.entries());
}

function elementEntries(
  elements: ReadonlyMap<string, BroadsetElement>,
): readonly (readonly [string, BroadsetElement])[] {
