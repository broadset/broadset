...
function instrumentInsertBefore(elementLayer: Element, onInsert: () => void): void {
  const originalInsertBefore: Element['insertBefore'] = elementLayer.insertBefore.bind(elementLayer);

  elementLayer.insertBefore = function instrumentedInsertBefore<T extends Node>(
    newNode: T,
...
describe('renderer performance contract', () => {
  ...
  it('does not reparent root hosts when a single element updates', () => {
    ...
    instrumentInsertBefore(elementLayer, () => {
      insertBeforeCalls += 1;
    });
  ...
  it('performs exactly one root-level insertion when adding a single element', () => {
    ...
    controller.updateDocument(createDocument([...elements, createElement({ id: 'new-tail', type: 'rectangle' })]));
